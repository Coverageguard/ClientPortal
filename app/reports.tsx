...
export default function ReportsScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: subs } = await supabase.from('subcontractors').select('id, company_name, fein, carrier_name, policy_number, verification_status, created_at, has_peo, peo_name').eq('client_id', clientData.id).order('created_at', { ascending: false });
      ...
      if (subs) {
        setSubcontractors(subs.map(s => ({
          ...
          lastVerified: s.created_at ? s.created_at.split('T')[0] : null, blocked: false,
 ...
 <View style={{ flex: 2 }}><Text style={styles.subName}>{sub.name}</Text><Text style={styles.subFein}>FEIN: {sub.fein}</Text></View>
